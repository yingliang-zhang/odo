...
// in internal/ipc/server.go (/panel, /vision, /preview, /loop) — keep the
// two in sync. Slash user_messages journal no recall key, so text-tokenized
...
var auditSlashCommands = []string{"/panel", "/vision", "/preview", "/loop"}
