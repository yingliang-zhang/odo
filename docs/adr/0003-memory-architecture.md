...
> **Invariant 7 (amended):** Distill remains the only *cadence* for LM-influenced memory writes — the learner rides inside the fold, curation triggers off distill markers on note count/age. R-W2/W3/W4 change how the daemon *procures* those completions, not who owns the write: behind prefs `distill_via:` / `learner_via:` / `curator_via:` / `design_via:` the distiller, learner, curator, and Design-MoA may take one direct `moa.Query` (or `QueryWithTools` for bounded read-only tool loops) instead of an OMP one-shot. The transport swap neither creates a model write path nor moves the cadence — the daemon owns every memory write. Model output on either route is inert text until daemon-side parsers and gates pass (learner proposals wait for the panel gate; user.md waits for human `apply_memory`; curator JSON passes shape, topic-validity, and citation-liveness gates), and every moa-route request body is journaled as an exact-bytes receipt (`request_sha16`/`request_bytes` on the fold, curate, and design_lock markers). Invariant 1 stands unmodified: no agent process ever writes a memory layer.

## Amendments (2026-08-28, control-plane hardening W2, D4)

Recorded from the control-plane hardening design lock (`docs/design/control-plane-hardening-lock.md`, D4, ruling ④ Sol hybrid). This amendment lands the memory write contract the code already implements, so the document matches reality; it changes no code path:

> **Invariant 1 (reaffirmed):** Agents write to no memory layer, ever — the daemon is the sole writer of every layer. The 2026-08-10 todo-block exception stands unchanged as the only carve-out.
>
> **Write cadence (corrected):** the daemon-side panel-gated auto-apply of PROJECT-scope memory.md rules and project skills is the M4+ write cadence: learner proposals ride the batch, the review panel gates them, and accepted proposals apply under actor `auto_panel` — the stale 2026-08-15 formulation "learner proposals wait for human `apply_memory`" described the pre-gate code, not the shipped one (corrected in place above). `user.md` (global) and every cross-project layer remain strictly human-written, forever: no automatic proposal, no automatic apply. The auto paths enforce this at two layers — the scope hold refuses user.md proposals with `memory_update{layer:"apply", cause:"scope_held_for_human"}` before any consume, and the apply core treats an accepted auto-path user.md proposal as an invariant violation (fail-closed error, never `planUserApply`).
>
> **Deletion-class changes never auto-apply (new):** rules-audit flags (`review_action{action:"memory_audit_flag"}`) enter the learner prompt as evidence-only DATA; a retract-intent proposal citing one rides the batch as `intent:"retract"` after a daemon-side LLM-free vet (real flag row, rule still present, oscillation guard). An accepted retract intent is NEVER applied — it surfaces as `memory_update{layer:"memory", cause:"retract_candidate"}` and a human resolves it (apply_memory's contradiction path, or `odo rules retract`). A rule retracted then re-landed within 3 epochs freezes: retract proposals for it reject with `oscillation_guard`.
>
> **Rollback (new):** human-only `odo memory revert <epoch>` restores an epoch apply's pre-image from the journaled receipt chain and journals `memory_update{layer:"apply", cause:"revert", actor:"human"}`; a second revert of the same epoch is refused. Revert-of-revert is a re-apply and belongs to the normal path.
