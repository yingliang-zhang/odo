...
**Amendment 2026-08-24: memory paths refused at registration (fail-fast)**. The 2026-08-20 split shrank the hard blocks to memory + supply-chain, but pre-panel still ran fail-late for memory paths: the diff registered, burned a verify+panel cycle, then hit the terminal `protected_path` block — a permanent pending wedge, since the executor's `rejectMemoryPaths` refuses those bytes for EVERY actor including the human Accept click. `drainRun` now refuses such diffs at REGISTRATION (`git.PatchPaths` → `rejectMemoryPaths`, same predicate the accept-time backstop uses): no diff row is ever inserted, the run retires shaped exactly like the no-diff outcome, and a transcript-visible advisory names the protected path, the salvage patch (`<project>/.odo/diffs/<run>.diff` is kept verbatim), and the correct route — wiki/ notes land through the daemon's own distill/`commitWiki` pipeline, so the producer strips the memory hunk and resends. Loop-provenance runs get the failure matrix: the loop suspends `run_tainted` with the refusal detail (`runMeta.refusalDetail`), replacing the legacy `risk:protected_path` "land manually" advice which the every-actor refusal made a dead end. Supply-chain paths are unchanged (still register → blocked at pre-panel; loop Mode A still suspends `risk:supply_chain`), as are unparseable patches (pass through to the accept-time backstop). Pins: `TestMemoryDiffRefusedAtRegistration` (wiki/ + .odo/ hunks; no row, advisory, salvage, retire), `TestLoopRiskGateSuspends` (protected-memory case re-pinned to `run_tainted`).

**Loop pipeline unchanged**: `/loop` fix rounds land with `loopActor` and no panel — `panelVerdictAttestsDiff` structurally refuses a loopActor gate-source land (no auto_panel verdict rows exist for it). Memory-path fix diffs never reach that gate: since the 2026-08-24 amendment they are refused at registration and the loop suspends `run_tainted` (see above).
