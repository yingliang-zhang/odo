...
var table = map[string]Spec{
	...
	// glm-5.3 (user ruling, 2026-08-29): window 1M, CompactRatio 0.5 →
	// 500K trigger. The 0.35 global ratio was the first guess; the user
	// sized the trigger to half the window deliberately — earlier Hermes
	// sessions measured the Sudo GLM relay's real prefill comfort ceiling
	// well below the nominal window, and a mid-window trigger keeps
	// compaction away from that wall. Thinking budget starts at 32768
	// like the other thinking models (glm-5.2's 16384 start predates the
	// measured thinking-trace burn; moa escalates ×2 up to MaxOutput on
	// stop_reason=max_tokens either way).
	"glm-5.3": {ContextWindow: 1000000, CompactRatio: 0.50, MaxOutput: 65536, MaxTokens: 32768},
}
